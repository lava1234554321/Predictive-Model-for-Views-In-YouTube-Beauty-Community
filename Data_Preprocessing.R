...
#################################################Read in Comments###################################
mydata2$comments = ""
for(i in list.files(path = "~/Comments_2", full.names = TRUE)) {
  print(i)
...
faces = unique(faces)
mydata2 = merge(x = mydata2, y = faces, by.x="link", all.x = TRUE)
...
emoji$format = result
format_comments=mclapply(mydata2$comments, FUN = ununemoji, mc.cores = 4)
mydata2$format_comments = format_comments

format_desc = lapply(mydata2$desbox, FUN = ununemoji)
mydata2$format_desbox = format_desc
format_title = lapply(mydata2$title, FUN = ununemoji)
mydata2$format_title = format_title
#This step will take a long time##

########################################Adding More Variables##################################
mydata2$tag_new = lapply(X= mydata2$tags, FUN = tagtovector)
mydata2$pubdate = gsub("Uploaded on ", "", mydata2$pubdate)
mydata2$day_of_week = weekdays((as.Date(mydata2$pubdate, format = "%B %d, %Y")))
mydata2$pubdate = (as.Date(mydata2$pubdate, format = "%B %d, %Y"))
mydata2$year = substr(mydata2$pubdate, 1, 4)
mydata2$month = substr(mydata2$pubdate, 6, 7)
mydata2$day = substr(mydata2$pubdate, 9, 10)
mydata2$ratio = mydata2$likes/mydata2$dislikes
########################################Stuff in Title ##################################
mydata2$title = tolower(mydata2$title)
...
########################################Certain Tags##################################
mydata2$tags = tolower(mydata2$tags)
...
mydata2$deslen = nchar(mydata2$desbox)
...
com_pol_result = mclapply(mydata2$format_comments, FUN = comAvePol2, mc.cores = 4)
